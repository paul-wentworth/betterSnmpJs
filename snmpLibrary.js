...
function readValue(type, reader) // [TODO] - uint wrapper
{
	...
	if( type == DATATYPES.Boolean )          { value = reader.readBoolean(); }
			 ...
			 type == DATATYPES.Gauge   ||
			 type == DATATYPES.TimeTicks)    { value = reader.readString(type, true); }
...
class pdu
{
	...
	serialize(writer)
	{
		...
		for( let i = 0; i < this.varbinds.length; i++ )
		{
			...
			writer.writeOID(this.varbinds[i][0][ Object.keys(this.varbinds[i][0])[0] ]); // array of arrays of {"key" : value}
			
			let value = this.varbinds[i][1][ Object.keys(this.varbinds[i][1])[0] ];
